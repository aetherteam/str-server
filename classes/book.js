...
module.exports = {
  create: async function (
    ...
    if (!validation.basic(name)) {
      errors.push({
        type: "name",
        text: "Название книги не может быть пустым или превышать 150 символов",
      });
    }
    ...
    if (!validation.bookDescription(description)) {
      errors.push({
        type: "description",
        text: "Длина описания не может быть больше 1000 символов",
      });
    }
    ...
    if (errors.length > 0) {
      console.log("[Book] one or more fields was invalid");
  ...
  publishChapter: async function (chapterID) {
    const booksCollection = global.mongo.collection("books");
    const chaptersCollection = global.mongo.collection("chapters");
    const doUserHaveRights = await module.exports.checkOwnership(
      bookID,
    ...
    const chapter = chaptersCollection.getOne({
      id: chapterID,
    });
    if (!chapter) {
  ...
  //TODO: SHOW CHAPTER DRAFTS
  getOne: async function (bookID, userID, countAsView, showChapterDrafts=false) {
    const chaptersCollection = global.mongo.collection("chapters");
    const booksCollection = global.mongo.collection("books");
    ...
    if (countAsView) {
      module.exports.addView(bookID, userID);
    }

    return results.successWithData(book);
  },
  get: async function (arrangeType, shift = 0, count = 10, showChapterDrafts=false) {
    // const chaptersCollection = global.mongo.collection("chapters");
    const booksCollection = global.mongo.collection("books");
  ...
  getByAuthor: async function (authorID, showChapterDrafts=false) {
    // const chaptersCollection = global.mongo.collection("chapters");
    const booksCollection = global.mongo.collection("books");

    const cursor = await booksCollection.find({ author: parseInt(authorID) });
  ...
  like: async function (userID, bookID) {
    ...
    let result;
    if (!doUserLiked) {
      result = await bookLikesCollection.insertOne({
        userID,
        bookID,
      });
      if (result) {
      ...
      result = await bookLikesCollection.deleteOne({
        userID,
        bookID,
      });
      if (result) {
  ...
  addView: function (elementID, userID, where) {
    let collection;
    switch (where) {
      case "chapter":
        collection = global.mongo.collection("chapterViews");
        break;
      case "book":
        collection = global.mongo.collection("bookViews");
        break;
    }

    const doUserViewed = collection.getOne({
      userID,
      elementID,
    });
    ...
    console.log("Element" + elementID + "viewed by" + userID);
